...
class Controller(BaseController):
    def __init__(self, scenario, sessions, chat_id=None, session_names=(None, None)):
        super(Controller, self).__init__(
            scenario, sessions, chat_id, session_names=session_names
        )
    ...
    def get_outcome(self):
        ...
        if self.offers[0] is not None and self.outcomes[1] is True:
            ...
            if (
                self.offers[0] is not None or self.offers[1] is not None
            ) and False in self.outcomes:
                reward = 0
    ...
    def game_over(self):
        return not self.inactive() and (
            (self.offers[0] is not None and self.outcomes[1] is not None)
            or (self.offers[1] is not None and self.outcomes[0] is not None)
            or self.quit
        )
    ...
    def complete(self):
        return (self.offers[0] is not None and self.outcomes[1] is True) or (
            self.offers[1] is not None and self.outcomes[0] is True
        )
