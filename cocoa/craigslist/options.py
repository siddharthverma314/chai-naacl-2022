...
def add_preprocess_arguments(parser):
    parser.add_argument(
        "--entity-encoding-form",
        choices=["canonical", "type"],
        default="canonical",
        help="Input entity form to the encoder",
    )
    parser.add_argument(
        "--entity-decoding-form",
        choices=["canonical", "type"],
        default="canonical",
        help="Input entity form to the decoder",
    )
    parser.add_argument(
        "--entity-target-form",
        choices=["canonical", "type"],
        default="canonical",
        help="Output entity form to the decoder",
    )
    parser.add_argument(
        "--cache", default=".cache", help="Path to cache for preprocessed batches"
    )
    parser.add_argument(
        "--ignore-cache", action="store_true", help="Ignore existing cache"
    )
...
def add_model_arguments(parser):
    ...
    group.add_argument(
        "-copy_attn", action="store_true", help="Train copy attention layer."
    )
    group.add_argument(
        "--layers", type=int, default=-1, help="Number of layers in enc/dec."
    )
    group.add_argument(
        "--enc-layers", type=int, default=1, help="Number of layers in the encoder"
    )
    group.add_argument(
        "--dec-layers", type=int, default=1, help="Number of layers in the decoder"
    )
    group.add_argument(
        "--rnn-size", type=int, default=500, help="Size of rnn hidden states"
    )
    group.add_argument(
        "--rnn-type",
        type=str,
...
def add_website_arguments(parser):
    parser.add_argument(
        "--port", type=int, default=5000, help="Port to start server on"
    )
    parser.add_argument(
        "--host",
        type=str,
