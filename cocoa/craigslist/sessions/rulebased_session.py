...
from cocoa.craigslist.core.tokenizer import tokenize, detokenize
from cocoa.craigslist.model.parser import Parser, Utterance
...
class RulebasedSession(object):
    ...
    def get_session(cls, agent, kb, lexicon, config, generator, manager):
        if kb.role == "buyer":
            ...
            return SellerRulebasedSession(
                agent, kb, lexicon, config, generator, manager
            )
        else:
...
Config = namedtuple(
    "Config",
    ["overshoot", "bottomline_fraction", "compromise_fraction", "good_deal_threshold"],
)

default_config = Config(0.2, 0.5, 0.5, 0.7)
...
class CraigslistRulebasedSession(BaseRulebasedSession):
    def __init__(self, agent, kb, lexicon, config, generator, manager):
        ...
        super(CraigslistRulebasedSession, self).__init__(
            agent, kb, parser, generator, manager, state, sample_temperature=10.0
        )
    ...
    def get_fraction(cls, zero, one, fraction):
        ...
        """
        return one * fraction + zero * (1.0 - fraction)
    ...
    def fill_template(self, template, price=None):
        return template.format(
            title=self.title,
            price="$PRICE=" + (str(price) or "None"),
            listing_price="$LIST_PRICE=" + str(self.listing_price),
            partner_price="$PARTNER_PRICE=" + (str(self.state.partner_price) or ""),
            my_price="$MY_PRICE=" + (str(self.state.my_price) or ""),
        )

    def template_message(self, intent, price=None):
        template = self.retrieve_response_template(
            intent, category=self.kb.category, role=self.kb.role
        )
    ...
    def _compromise_price(self, price):
        partner_price = (
            self.state.partner_price
            if self.state.partner_price is not None
            else self.bottomline
        )
        if partner_price is None:
            # TODO: make it a parameter
            my_price = price * (1 - self.inc * 0.1)
        else:
            my_price = self.get_fraction(
                partner_price, price, self.config.compromise_fraction
            )
    ...
    def compromise(self):
        if (
            self.bottomline is not None
            and self.compare(self.state.my_price, self.bottomline) <= 0
        ):
        ...
        if (
            self.state.partner_price
            and self.compare(self.state.my_price, self.state.partner_price) < 0
        ):
    ...
    def offer(self, price):
        ...
        return super(BaseRulebasedSession, self).offer(
            {"price": price}, metadata=metadata
        )
    ...
    def deal(self, price):
        if self.bottomline is None:
            return False
        good_price = self.get_fraction(
            self.bottomline, self.target, self.config.good_deal_threshold
        )
        # Seller
        if self.inc == 1 and (
            price >= min(self.listing_price, good_price) or price >= self.state.my_price
        ):
            return True
        # Buyer
        if self.inc == -1 and (price <= good_price or price <= self.state.my_price):
            return True
        return False
    ...
    def no_deal(self, price):
        ...
        if self.bottomline is not None:
            if (
                self.compare(price, self.bottomline) < 0
                and abs(price - self.bottomline) > 1
            ):
                return True
        else:
            return True
    ...
    def send(self):
        ...
        if self.has_done("final_call"):
            return self.offer(
                self.bottomline
                if self.compare(self.bottomline, self.state.partner_price) > 0
                else self.state.partner_price
            )
...
class SellerRulebasedSession(CraigslistRulebasedSession):
    def __init__(self, agent, kb, lexicon, config, generator, manager):
        super(SellerRulebasedSession, self).__init__(
            agent, kb, lexicon, config, generator, manager
        )
        # Direction of desired price
        self.inc = 1.0
    ...
    def estimate_bottomline(self):
        if self.state.partner_price is None:
            ...
            return self.get_fraction(
                self.state.partner_price,
                self.listing_price,
                self.config.bottomline_fraction,
            )
...
class BuyerRulebasedSession(CraigslistRulebasedSession):
    def __init__(self, agent, kb, lexicon, config, generator, manager):
        super(BuyerRulebasedSession, self).__init__(
            agent, kb, lexicon, config, generator, manager
        )
        # Direction of desired price
        self.inc = -1.0
    ...
    def estimate_bottomline(self):
        return self.get_fraction(
            self.listing_price, self.target, self.config.bottomline_fraction
        )

    def init_price(self):
        self.state.my_price = self.round_price(
            self.target * (1 + self.inc * self.config.overshoot)
        )
