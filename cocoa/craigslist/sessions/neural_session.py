...
from cocoa.craigslist.sessions.session import Session
from cocoa.craigslist.neural.preprocess import markers, Dialogue
from cocoa.craigslist.neural.batcher import Batch
...
class NeuralSession(Session):
    def __init__(self, agent, kb, env):
        ...
        self.kb_context_batch = self.batcher.create_context_batch(
            [self.dialogue], self.batcher.kb_pad
        )
        self.max_len = 100
    ...
    def convert_to_int(self):
        for i, turn in enumerate(self.dialogue.token_turns):
            for curr_turns, stage in zip(
                self.dialogue.turns, ("encoding", "decoding", "target")
            ):
                if i >= len(curr_turns):
...
class PytorchNeuralSession(NeuralSession):
    ...
    def _create_batch(self):
        ...
        # All turns up to now
        self.convert_to_int()
        encoder_turns = self.batcher._get_turn_batch_at(
            [self.dialogue], Dialogue.ENC, None
        )
    ...
    def generate(self):
        ...
        output_data = self.generator.generate_batch(
            batch, gt_prefix=self.gt_prefix, enc_state=enc_state
        )
