...
from cocoa.craigslist.sessions.rulebased_session import CraigslistRulebasedSession
...
class BaseHybridSession(CraigslistRulebasedSession):
    def __init__(self, agent, kb, lexicon, config, generator, manager):
        super(BaseHybridSession, self).__init__(
    ...
    def receive(self, event: Event):
        ...
        if event.action == "message":
            ...
            entity_tokens = self.manager.env.preprocessor.lf_to_tokens(
                self.kb, logical_form
            )
        else:
    ...
    def is_valid_action(self, action_tokens):
        if not action_tokens:
            return False
        if action_tokens[0] in self.price_actions and not (
            len(action_tokens) > 1 and is_entity(action_tokens[1])
        ):
            return False
        return True
    ...
    def send(self):
        ...
        if not self.is_valid_action(action_tokens):
            ...
            if action in self.price_actions:
                ...
                # TODO: It seems like partner_price gets updated when the manager likes the counter-offer price
                price = self.state.partner_price
...
class SellerHybridSession(BaseHybridSession):
    def __init__(self, agent, kb, lexicon, config, generator, manager):
        super(SellerHybridSession, self).__init__(
            agent, kb, lexicon, config, generator, manager
        )
        self.inc = 1.0
        self.init_price()
...
class BuyerHybridSession(BaseHybridSession):
    def __init__(self, agent, kb, lexicon, config, generator, manager):
        super(BuyerHybridSession, self).__init__(
            agent, kb, lexicon, config, generator, manager
        )
        self.inc = -1.0
        self.init_price()
