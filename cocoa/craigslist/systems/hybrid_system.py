...
from cocoa.craigslist.sessions.hybrid_session import HybridSession
...
class HybridSystem(BaseRulebasedSystem):
    def _new_session(self, agent, kb, config=None):
        ...
        return HybridSession.get_session(
            agent, kb, self.lexicon, self.generator, manager_session
        )
