...
from cocoa.craigslist.neural.batcher import DialogueBatcherFactory, Batch
from cocoa.craigslist.neural.symbols import markers
from cocoa.craigslist.neural.vocab_builder import create_mappings
from cocoa.craigslist.neural import make_model_mappings
...
class TextIntMap(object):
    ...
    def pred_to_input(self, preds, prices=None):
        ...
        input_utterances = [
            self.preprocessor.process_utterance(utterance, "decoding")
            for utterance in preds_utterances
        ]
        inputs = np.array(
            [self.text_to_int(utterance, "decoding") for utterance in input_utterances]
        )
        return inputs
    ...
    def int_to_text(self, inds, stage=None, prices=None):
        ...
        if prices is not None:
            ...
            toks = [
                CanonicalEntity(value=p, type="price") if price_filler(x) else x
                for x, p in zip(toks, prices)
            ]
        return toks
...
class Dialogue(object):
    ...
    def __init__(self, agent, kb, uuid, model="seq2seq"):
        ...
        self.description = tokenize(
            re.sub(r"[^\w0-9]", " ", " ".join(kb.facts["item"]["Description"]))
        )
    ...
    def scale_price(cls, kb, utterance):
        return [
            PriceScaler.scale_price(kb, x) if is_entity(x) else x for x in utterance
        ]
    ...
    def get_price_turns(self, pad):
        ...
        prices = [
            [to_float_price(entity) if entity else pad for entity in entities]
...
class Preprocessor(object):
    ...
    def __init__(
        ...
        schema,
        lexicon,
        entity_encoding_form,
        entity_decoding_form,
        entity_target_form,
        model="seq2seq",
    ):
        ...
        if stage is None:
            ...
            if stage == "encoding":
                ...
                if self.model == "sum2sum":
                    ...
                    summary = utterance
            return [
                self.get_entity_form(x, self.entity_forms[stage]) if is_entity(x) else x
                for x in summary
            ]
    ...
    def process_event(self, e, kb):
        ...
        if e.action == "message":
            ...
            entity_tokens = self.lexicon.link_entity(
                tokenize(e.data), kb=kb, scale=True, price_clip=4.0
            )
            if entity_tokens:
...
class DataGenerator(object):
    def __init__(
        ...
        if (not os.path.exists(cache)) or ignore_cache:
            ...
            self.dialogues = {
                k: preprocessor.preprocess(v) for k, v in examples.iteritems() if v
            }

            for fold, dialogues in self.dialogues.iteritems():
                print(
                    "%s: %d dialogues out of %d examples"
                    % (fold, len(dialogues), self.num_examples[fold])
                )
        else:
        ...
        self.dialogue_batcher = DialogueBatcherFactory.get_dialogue_batcher(
            model,
        ...
        self.batches = {
            k: self.create_batches(k, dialogues, batch_size)
    ...
    def load_mappings(self, model_type, mappings_path, schema, preprocessor):
        ...
        if not os.path.exists(vocab_path):
            ...
            mappings = create_mappings(
                self.dialogues["train"], schema, preprocessor.entity_forms.values()
            )
