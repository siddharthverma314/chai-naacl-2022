...
from cocoa.neural.models import (
    MeanEncoder,
    StdRNNEncoder,
    StdRNNDecoder,
    MultiAttnDecoder,
...
from cocoa.craigslist.neural.models import NegotiationModel
...
def build_model(model_opt, opt, fields, checkpoint):
    print("[model_builder.py] Building model...")
    model = onmt.ModelConstructor.make_base_model(
        model_opt, fields, use_gpu(opt), checkpoint
    )
    if len(opt.gpuid) > 1:
        print("[model_build.py] Multi gpu training: ", opt.gpuid)
...
def make_encoder(opt, embeddings):
    ...
    if opt.encoder_type == "transformer":
        ...
        return CNNEncoder(
            opt.enc_layers, opt.rnn_size, opt.cnn_kernel_width, opt.dropout, embeddings
        )
...
def load_test_model(model_path, opt, dummy_opt):
    ...
    mappings = read_pickle(opt._vocab_pickle_path)
...
def make_base_model(model_opt, mappings, gpu, checkpoint=None):
    ...
    if "multibank" in model_opt.global_attention:
        model = NegotiationModel(
            encoder, decoder, context_embedder, kb_embedder, stateful=model_opt.stateful
        )
    else:
    ...
    if not model_opt.copy_attn:
        generator = nn.Sequential(
            nn.Linear(model_opt.rnn_size, len(tgt_dict)), nn.LogSoftmax()
        )
    ...
    if checkpoint is not None:
        # print("[model_builder.py] Loading model parameters.")
        model.load_state_dict(checkpoint["model"])
        generator.load_state_dict(checkpoint["generator"])
    else:
        if model_opt.param_init != 0.0:
            print("[model_builder.py] Intializing model parameters.")
