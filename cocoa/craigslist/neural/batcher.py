...
class DialogueBatcher(object):
    ...
    def _get_turn_batch_at(self, dialogues, STAGE, i):
        ...
        if i is None:
            # Return all turns
            return [
                self._get_turn_batch_at(dialogues, STAGE, i)
                for i in range(dialogues[0].num_turns)
            ]
        else:
    ...
    def create_context_batch(self, dialogues, pad):
        ...
        description_batch = pad_list_to_array(
            [[pad] if not d.description else d.description for d in dialogues],
            pad,
            np.int32,
        )
        return {
    ...
    def get_encoder_context(self, encoder_turns, num_context):
        ...
        encoder_context = [
            self._remove_prompt(turn)
            for turn in encoder_turns[-1 * (num_context + 1) : -1]
        ]
    ...
    def _create_one_batch(
        ...
        decoder_inputs, decoder_targets = self.make_decoder_inputs_and_targets(
            decoder_turns, target_turns
        )
    ...
    def int_to_text(self, array, textint_map, stage):
        tokens = [
            str(x)
            for x in textint_map.int_to_text((x for x in array if x != self.pad), stage)
        ]
        return " ".join(tokens)
    ...
    def print_batch(self, batch, example_id, textint_map, preds=None):
        ...
        print(
            "ENC INPUT:\n {}".format(
                self.int_to_text(
                    batch["encoder_args"]["inputs"][i], textint_map, "encoding"
                )
            )
        )
        print(
            "DEC INPUT:\n {}".format(
                self.int_to_text(
                    batch["decoder_args"]["inputs"][i], textint_map, "decoding"
                )
            )
        )
        print(
            "TARGET:\n {}".format(
                self.int_to_text(
                    batch["decoder_args"]["targets"][i], textint_map, "target"
                )
            )
        )
    ...
    def _get_token_turns_at(self, dialogues, i):
        ...
        return [
            dialogue.token_turns[i] if i < len(dialogue.token_turns) else ""
            for dialogue in dialogues
        ]
