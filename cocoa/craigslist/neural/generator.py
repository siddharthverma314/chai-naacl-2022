...
from cocoa.craigslist.neural.symbols import markers, category_markers, sequence_markers
from cocoa.craigslist.neural.utterance import UtteranceBuilder


class LFSampler(Sampler):
    def __init__(self, model, vocab, temperature=1, max_length=100, cuda=False):
        super(LFSampler, self).__init__(
            model, vocab, temperature=temperature, max_length=max_length, cuda=cuda
        )
        self.price_actions = map(
            self.vocab.to_ind, ("init-price", "counter-price", markers.OFFER)
        )
        self.prices = set(
            [id_ for w, id_ in self.vocab.word_to_ind.items() if is_entity(w)]
        )
        ...
        actions = set(
            [
                w
                for w in self.vocab.word_to_ind
                if not (
                    is_entity(w)
                    or w in category_markers
                    or w in sequence_markers
    ...
    def generate_batch(self, batch, gt_prefix=1, enc_state=None):
        ...
        for i in range(self.max_length):
            ...
            out = self.model.generator.forward(
                dec_out
            ).data  # Logprob (batch_size, vocab_size)
            ...
            scores.sub_(
                scores.max(1, keepdim=True)[0].expand(scores.size(0), scores.size(1))
            )
