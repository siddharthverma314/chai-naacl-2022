...
class Parser(BaseParser):
    ...
    stopwords.update(
        ["may", "might", "rent", "new", "brand", "low", "high", "now", "available"]
    )
        ...
        price_types = list(
            set(
                [
                    ...
                    for token in tokens_with_parsed_price
    ...
    def get_proposed_price(self, tokens, dialogue_state):
        ...
        inc = 1.0 if partner_role == "seller" else -1.0
        for price in filter(lambda x: self.is_price_token(x), tokens):
            ...
            if (
                type_ != "curr_price"
                and (
                    partner_prev_price is None
                    or self.compare(price, partner_prev_price, inc) <= 0
                )
                and (partner_role == "seller" or type_ != "listing_price")
            ):
    ...
    def parse_prices(self, tokens, dialogue_state):
        tokens = [
            self._parse_price(token, dialogue_state)
            if self.is_price_token(token)
            else token
            for token in tokens
        ]
    ...
    def extract_template(self, tokens, dialogue_state):
        ...
        tokens = [
            "{%s}" % token.canonical.type if self.is_price_token(token) else token
            for token in tokens
        ]
    ...
    def parse_message(self, event, dialogue_state):
        ...
        intent = self.classify_intent(
            utterance, tokens_with_parsed_price, dialogue_state
        )

        proposed_price = self.get_proposed_price(
            tokens_with_parsed_price, dialogue_state
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--transcripts", nargs="*", help="JSON transcripts to extract templates"
    )
    ...
    counter = {
        "total_lf": 0,
        "unk_lf": 0,
        "seqs": defaultdict(lambda: defaultdict(lambda: defaultdict(int))),
    }
    for example in examples:
        if Preprocessor.skip_example(example):
            continue
        parse_dialogue(example, price_tracker, counter)
    print("[parser.py] % unk:", counter["unk_lf"] / float(counter["total_lf"]))
    print("[parser.py] intent seqs:")
    seqs = counter["seqs"]
    for first in seqs:
        ...
        print("[parser.py]", first, "unk={:.3f}".format(unks))
        for second in seqs[first]:
            ...
            print(
                ...
                first,
                second,
                "{:.3f}".format(total / float(total_first)),
                "unk={:.3f}".format(unks),
            )
        print("[parser.py] ------------------------")
