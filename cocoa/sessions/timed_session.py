...
class TimedSessionWrapper(Session):
    ...
    def send(self):
        ...
        if self.num_utterances >= 1:
            return None
        if self.received is False and (
            self.prev_action == "select"
            or self.last_message_timestamp + random.uniform(1, self.PATIENCE)
            > time.time()
        ):
            return None
        ...
        if self.last_message_timestamp + delay > time.time():
            ...
            if (
                ...
                and self.start_typing is False
                and self.last_message_timestamp + reading_time < time.time()
            ):
