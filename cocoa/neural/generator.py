...
from cocoa.neural.beam import Beam
...
class Generator(object):
    ...
    def _run_encoder(self, batch, enc_states=None):
        ...
        enc_states, memory_bank = self.model.encoder(
            encoder_inputs, lengths, enc_states
        )
    ...
    def generate_batch(self, batch, gt_prefix=1, enc_state=None):
        ...
        for i in range(self.max_length):
            ...
            for j, b in enumerate(beam):
                b.advance(
                    out[:, j], unbottle(attn["std"]).data[:, j, : memory_lengths[j]]
                )
