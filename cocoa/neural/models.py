...
from cocoa.neural.attention import (
    MultibankGlobalAttention,
    GlobalAttention,
    MultibankConcatGlobalAttention,
)
...
class RNNDecoderBase(nn.Module):
    ...
    def get_final_non_pad_output(self, tgt, outputs):
        ...
        final_output = torch.gather(outputs, 0, gather_ind).squeeze(
            0
        )  # (batch_size, rnn_size)
        return final_output
...
class StdRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(
        self, tgt, memory_banks, state, memory_lengths=None, lengths=None
    ):
        """
...
class NMTModel(nn.Module):
    ...
    def forward(
        self, src, tgt, lengths, dec_state=None, enc_state=None, tgt_lengths=None
    ):
...
class RNNDecoderState(DecoderState):
    def __init__(self, hidden_size, rnnstate):
        ...
        self.input_feed = Variable(
            self.hidden[0].data.new(*h_size).zero_(), requires_grad=False
        ).unsqueeze(0)
    ...
    def repeat_beam_size_times(self, beam_size):
        ...
        vars = [
            Variable(e.data.repeat(1, beam_size, 1), volatile=True) for e in self._all
        ]
...
class MultiAttnDecoder(StdRNNDecoder):
    def __init__(
        ...
        super(MultiAttnDecoder, self).__init__(
            rnn_type,
            bidirectional_encoder,
            num_layers,
            hidden_size,
...
class MultiAttnConcatDecoder(StdRNNDecoder):
    def __init__(
        ...
        attn_type = attn_type[10:]
        super(MultiAttnConcatDecoder, self).__init__(
            rnn_type,
            bidirectional_encoder,
            num_layers,
            hidden_size,
