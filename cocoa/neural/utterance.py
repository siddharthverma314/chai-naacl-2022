...
class Utterance(object):
    ...
    def __init__(self, src_raw, pred_sents, attn, pred_scores, tgt_sent, gold_score):
...
class UtteranceBuilder(object):
    ...
    def entity_to_str(self, entity_tokens, kb):
        return [
            self._entity_to_str(token, kb) if is_entity(token) else token
    ...
    def from_batch(self, translation_batch):
        ...
        for b in range(batch_size):
            ...
            if not batch.context_data["decoder_tokens"][b]:
                continue
            pred_sents = [
                self.build_target_tokens(preds[b][n]) for n in range(self.n_best)
            ]
            gold_sent = None
            if tgt is not None:
