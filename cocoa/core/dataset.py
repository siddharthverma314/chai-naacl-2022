...
class Example(object):
    ...
    def __init__(
        self, scenario, uuid, events, outcome, ex_id, agents, agents_info=None
    ):
    ...
    def from_dict(cls, raw, Scenario, scenario_db=None):
        ...
        return Example(
            scenario, uuid, events, outcome, ex_id, agents, agents_info=agents_info
        )
    ...
    def test_dict(cls, raw):
        ...
        agents_info = raw.get("agents_info", None)
        return Example(
            None, uuid, events, outcome, ex_id, agents, agents_info=agents_info
        )
...
class EvalExample(object):
    ...
    def __init__(
        self, uuid, kb, agent, role, prev_turns, prev_roles, target, candidates, scores
    ):
        self.ex_id = uuid
        self.kb = kb
        self.agent = agent
    ...
    def from_dict(schema, raw):
        ...
        scores = raw["results"]
        return EvalExample(
            ex_id, kb, agent, role, prev_turns, prev_roles, target, candidates, scores
        )
...
def read_dataset(args, Scenario):
    ...
    train_examples = read_examples(
        args.train_examples_paths, args.train_max_examples, Scenario
    )
    test_examples = read_examples(
        args.test_examples_paths, args.test_max_examples, Scenario
    )
    print(
        "We found {0} train examples and {1} test examples".format(
