...
class Controller(object):
    ...
    def __init__(
        ...
        self.session_status = {
            agent: "received" for agent, _ in enumerate(self.sessions)
        }
            ...
            for agent, session in enumerate(self.sessions):
                ...
                if verbose:
                    ...
                    event_output = (
                        data
                        if action == "message"
                        else "Action: {0}, Data: {1}".format(action, data)
                    )
        ...
        with self.lock:
            ...
            for agent, session in enumerate(self.sessions):
                if session is None:
                    ...
                    # inactive
                    continue
                if (not self.allow_cross_talk) and self.session_status[
                    agent
                ] != "received":
                    continue
                event = session.send()
