class Event(object):
    ...
    def MessageEvent(cls, agent, data, time=None, start_time=None, metadata=None):
        return cls(
            agent, time, "message", data, start_time=start_time, metadata=metadata
        )
        ...
        for e in events:
            if e.action == "eval":
                event_dict[e.time].tags = [
                    k for k, v in e.data["labels"].iteritems() if v != 0
                ]
            else:
                event_dict[e.time].tags = []
        events_with_eval = [
            v for k, v in sorted(event_dict.iteritems(), key=lambda x: x[0])
        ]
