...
class Trainer(object):
    ...
    def train(self, train_iter, epoch, report_func=None):
        ...
        for i, batch in enumerate(train_iter):
            ...
            if self.norm_method == "tokens":
                normalization += (
                    batch.tgt[1:].data.view(-1).ne(self.train_loss.padding_idx).sum()
                )
            else:
            ...
            if accum == self.grad_accum_count:
                self._gradient_accumulation(
                    true_batchs, total_stats, report_stats, normalization
                )
    ...
    def _gradient_accumulation(
        self, true_batchs, total_stats, report_stats, normalization
    ):
