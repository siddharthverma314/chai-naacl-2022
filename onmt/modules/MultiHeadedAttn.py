...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, head_count, model_dim, dropout=0.1):
        ...
        self.linear_keys = BottleLinear(
            model_dim, head_count * self.dim_per_head, bias=False
        )
        self.linear_values = BottleLinear(
            model_dim, head_count * self.dim_per_head, bias=False
        )
        self.linear_query = BottleLinear(
            model_dim, head_count * self.dim_per_head, bias=False
        )
    ...
    def forward(self, key, value, query, mask=None):
        ...
        def shape_projection(x):
            b, l, d = x.size()
            return (
                x.view(b, l, self.head_count, self.dim_per_head)
                .transpose(1, 2)
                .contiguous()
        ...
        def unshape_projection(x, q):
            b, l, d = q.size()
            return (
                x.view(b, self.head_count, l, self.dim_per_head)
                .transpose(1, 2)
                .contiguous()
        ...
        attn = self.sm(scaled)
        # Return one attn
        top_attn = attn.view(b, self.head_count, l, dim_per_head)[
            :, 0, :, :
        ].contiguous()
