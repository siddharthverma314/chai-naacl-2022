...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, input, memory_bank, src_pad_mask, tgt_pad_mask):
        ...
        dec_mask = torch.gt(
            tgt_pad_mask
            + self.mask[:, : tgt_pad_mask.size(1), : tgt_pad_mask.size(1)].expand_as(
                tgt_pad_mask
            ),
            0,
        )
        ...
        mid, attn = self.context_attn(
            memory_bank, memory_bank, query_norm, mask=src_pad_mask
        )
    ...
    def forward(self, tgt, memory_bank, state, memory_lengths=None):
        ...
        for i in range(self.num_layers):
            output, attn = self.transformer_layers[i](
                output, src_memory_bank, src_pad_mask, tgt_pad_mask
            )
